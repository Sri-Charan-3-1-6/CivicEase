...
const App: React.FC = () => {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto py-6 px-4 mb-24">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div className="text-left">
          ...
          <p className="text-xs md:text-lg text-gray-500 dark:text-gray-400 font-medium mt-2">Resume your previous tasks.</p>
        </div>
        <button onClick={() => setMode(Mode.IDLE)} className="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-primary dark:text-white bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-sm">{t('close')}</button>
      </div>
      <div className="grid grid-cols-1 gap-4">
        {sessions.length === 0 && <p className="text-center text-gray-400 font-bold py-10">No history found.</p>}
        {sessions.map(session => (
          <div key={session.id} onClick={() => resumeSession(session)} className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800 hover-lift cursor-pointer flex justify-between items-center group">
            <div className="flex gap-4 items-center">
              ...
              <div>
                ...
                <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mt-0.5">{session.timestamp}</p>
              </div>
            </div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto py-6 px-4 mb-24">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div className="text-left">
          ...
          <p className="text-xs md:text-lg text-gray-500 dark:text-gray-400 font-medium mt-2">{t('securedBy')} DigiLocker Gateway</p>
        </div>
        <button onClick={() => setMode(Mode.IDLE)} className="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-primary dark:text-white bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-sm">{t('close')}</button>
      </div>

      {!isLockerLinked ? (
        <div className="bg-white dark:bg-slate-900 p-12 rounded-[2.5rem] border-2 border-dashed border-gray-100 dark:border-slate-800 text-center relative overflow-hidden">
          ...
          <p className="text-xs text-gray-500 max-w-md mx-auto mb-8">{t('lockerRequiredDesc')}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {vaultDocs.map((doc) => (
            <div key={doc.id} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-gray-100 dark:border-slate-800 flex flex-col group">
              <div className="flex justify-between items-center mb-6">
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">{doc.status === 'FETCHED' ? t('govtVerified') : t('awaitingLink')}</span>
                  </div>
                </div>
                {doc.status !== 'FETCHED' ? (
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fcfcfd] dark:bg-[#020617] transition-colors duration-500 pb-20 md:pb-0 overflow-x-hidden">
      {/* Real-time Ticker */}
      <div className="w-full bg-primary-light dark:bg-slate-950 py-2.5 overflow-hidden flex items-center border-b border-white/5 z-[110]">
      ...
      {/* Top Nav (Desktop) */}
      <nav className="w-full glass sticky top-0 z-[100] px-6 md:px-12 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 md:gap-4">
           ...
           <button onClick={() => setIsLangOpen(true)} className="h-10 px-4 rounded-xl flex items-center gap-2 bg-slate-50 dark:bg-slate-800 text-primary dark:text-white border border-gray-100 dark:border-slate-700 transition-all hover:border-secondary shadow-sm">
      ...
      {isLangOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-100 dark:border-slate-800">
            ...
            <div className="p-4 grid grid-cols-2 gap-3 max-h-[60dvh] overflow-y-auto">
              {LANGUAGES.map(lang => (
                <button 
                  ...
                  className={`p-4 rounded-2xl text-left border transition-all ${language === lang.code ? 'bg-primary text-white border-primary shadow-lg' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-gray-600 dark:text-gray-300 hover:border-secondary'}`}
                >
      ...
      <main className="w-full flex-1 flex flex-col items-center py-10 px-4">
        <div className="w-full max-w-6xl">
          {mode === Mode.DIGILOCKER ? renderVault() : mode === Mode.HISTORY ? renderHistory() : mode === Mode.IDLE ? (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
              <div className="text-center space-y-8 max-w-5xl mx-auto mb-16">
                ...
                <div className="flex flex-wrap justify-center gap-4 mt-10">
                   <div className="px-6 py-3 bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 flex items-center gap-3 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={idx} onClick={() => startNewSession(item.m)} className="group bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-gray-100 dark:border-slate-800 flex flex-col min-h-[250px] relative overflow-hidden active:scale-95 transition-all duration-500 cursor-pointer hover:shadow-2xl">
                    <div className="absolute top-8 right-8 px-3 py-1 rounded-full bg-slate-50 dark:bg-slate-800 text-[8px] font-black uppercase tracking-widest text-gray-400 border border-gray-100 dark:border-slate-800">{t(item.badge)}</div>
                    <div className="bg-slate-50 dark:bg-slate-800 w-16 h-16 rounded-2xl flex items-center justify-center mb-8 shadow-inner group-hover:scale-110 transition-transform"><i className={`fas ${item.icon} text-2xl text-secondary`}></i></div>
                    <h3 className="text-2xl md:text-3xl font-black text-primary dark:text-white mb-2 tracking-tighter">{t(item.t)}</h3>
                    <p className="text-gray-400 dark:text-gray-500 text-xs md:text-base font-bold leading-relaxed mb-10 max-w-sm">{t(item.d)}</p>
            ...
            <div className="max-w-4xl mx-auto w-full"><ChatWindow initialMode={mode} language={language} vaultDocs={vaultDocs} initialMessages={activeSession?.messages || []} onMessagesChange={updateActiveSession} onReset={() => { setMode(Mode.IDLE); setActiveSession(null); }} onComplete={() => { setMode(Mode.IDLE); setActiveSession(null); }} onFetchDoc={handleFetchDoc} userLocation={userLocation}/></div>
          )}
        </div>
      </main>

      {/* Mobile Bottom Navigation */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 glass border-t border-gray-200 dark:border-slate-800 pb-[env(safe-area-inset-bottom)] z-[150] flex justify-around p-3">
          <button onClick={() => setMode(Mode.IDLE)} className={`flex flex-col items-center gap-1 flex-1 ${mode === Mode.IDLE || (mode !== Mode.DIGILOCKER && mode !== Mode.HISTORY) ? 'text-secondary' : 'text-gray-400'}`}>
              <i className="fas fa-home text-lg"></i>
              <span className="text-[8px] font-black uppercase tracking-wider">Home</span>
          </button>
          <button onClick={() => setMode(Mode.DIGILOCKER)} className={`flex flex-col items-center gap-1 flex-1 ${mode === Mode.DIGILOCKER ? 'text-secondary' : 'text-gray-400'}`}>
              <i className="fas fa-shield-halved text-lg"></i>
              <span className="text-[8px] font-black uppercase tracking-wider">Vault</span>
          </button>
          <button onClick={() => setMode(Mode.HISTORY)} className={`flex flex-col items-center gap-1 flex-1 ${mode === Mode.HISTORY ? 'text-secondary' : 'text-gray-400'}`}>
              <i className="fas fa-clock-rotate-left text-lg"></i>
              <span className="text-[8px] font-black uppercase tracking-wider">History</span>
          </button>
      </div>

      <footer className="w-full py-12 md:py-20 px-6 bg-slate-50 dark:bg-slate-900/50 border-t border-gray-100 dark:border-slate-800 flex flex-col items-center gap-8 md:gap-12 text-center mb-16 md:mb-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-24 max-w-4xl w-full">
              <div className="flex flex-col items-center md:items-start gap-3">
                  <span className="text-[6px] md:text-[10px] font-black uppercase tracking-[0.4em] text-gray-400">{t('footerDevCraft')}</span>
                  <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
                      <h4 className="text-[8px] md:text-sm font-black text-primary dark:text-white uppercase border border-gray-200 dark:border-slate-800 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 shadow-sm">{t('footerMadeIn')}</h4>
                      <h4 className="text-[8px] md:text-sm font-black text-primary dark:text-white uppercase border border-gray-200 dark:border-slate-800 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 shadow-sm">{t('footerMakeIn')}</h4>
                  </div>
              </div>
              <div className="flex flex-col items-center md:items-end gap-3">
                  <span className="text-[6px] md:text-[10px] font-black uppercase tracking-[0.4em] text-gray-400">{t('footerTrust')}</span>
                  <div className="flex flex-col md:flex-row items-center gap-4">
                      <h4 className="text-[8px] md:text-sm font-black text-primary dark:text-white uppercase border border-gray-200 dark:border-slate-800 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 shadow-sm">{t('footerTrusted')}</h4>
                  </div>
              </div>
          </div>

          <div className="flex flex-col items-center gap-2">
              <span className="text-[6px] md:text-[10px] font-black tracking-[0.6em] text-gray-400 uppercase">{t('footerPowered')}</span>
              <h5 className="text-xs md:text-3xl font-black text-primary dark:text-white tracking-tighter italic uppercase">GEMINI <span className="text-secondary">3.0</span> NATIVE</h5>
          </div>

          <p className="text-[6px] md:text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 opacity-50 pt-4">© 2025 CIVICEASE INDIA • ALL RIGHTS SECURED</p>
      </footer>
    </div>
  );
};
